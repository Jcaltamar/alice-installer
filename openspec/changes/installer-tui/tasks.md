...
- [ ] T-056 — **(GREEN)** `internal/tui/pull.go`: `PullState` consuming `PullProgress` msgs from `ComposeRunner` — *REQ-CO-2, REQ-TUI-4*. **Done**: T-055 passes.
- [ ] T-057 — **(RED)** `internal/tui/deploy_test.go` (teatest): "Starting services…"; service exits immediately → name+exit code+log lines + rollback prompt; port conflict at up-time → error screen; `q` → rollback; success → advances — *REQ-CO-3, REQ-CO-5, REQ-TUI-2, REQ-TUI-5*. **Done**: FAIL.
- [ ] T-058 — **(GREEN)** `internal/tui/deploy.go`: `DeployState` — *REQ-CO-3, REQ-CO-5*. **Done**: T-057 passes.
- [ ] T-059 — **(RED)** `internal/tui/healthcheck_test.go` (teatest): "2/3 healthy" progress bar; timeout at 120s → per-service status + view-logs binding; no-healthcheck service → "healthy (no check)" — *REQ-CO-4, REQ-TUI-4*. **Done**: FAIL.
- [ ] T-060 — **(GREEN)** `internal/tui/healthcheck.go`: `HealthcheckState` polling via `HealthTick` ticks — *REQ-CO-4*. **Done**: T-059 passes.
- [ ] T-061 — **(RED)** `internal/tui/result_test.go` (teatest): success screen green; partial-success shows failed services; `q` exits 0; exit code 1 on partial failure — *REQ-TUI-1*. **Done**: FAIL.
- [ ] T-062 — **(GREEN)** `internal/tui/result.go`: `ResultState` — *REQ-TUI-1*. **Done**: T-061 passes.
- [ ] T-063 — **(RED)** `internal/tui/resize_test.go` (teatest): `tea.WindowSizeMsg{Width:60, Height:20}` → plain-text "Terminal too small" (no lipgloss); resize to 82×26 → normal render resumes; mid-pull resize preserves progress state — *REQ-TUI-6, REQ-TUI-8*. **Done**: FAIL.
- [ ] T-064 — **(GREEN)** `internal/tui/model.go`: root `Model` with window-size guard; re-layout on `tea.WindowSizeMsg` — *REQ-TUI-6, REQ-TUI-8*. **Done**: T-063 passes.
- [ ] T-065 — **(RED)** `internal/tui/tty_test.go`: mock stdin non-TTY → `ErrNonTTY`; TTY → no error — *REQ-TUI-7*. **Done**: FAIL.
- [ ] T-066 — **(GREEN)** `internal/tui/tty.go`: `IsTTY(f *os.File) bool` + check in `cmd/installer/main.go` before `tea.NewProgram` — *REQ-TUI-7*. **Done**: T-065 passes.
- [ ] T-067 — **(RED)** `internal/tui/flow_test.go` (teatest integration): full happy-path flow using ALL fakes wired; asserts state sequence splash→preflight→workspace→portscan→envwrite→pull→deploy→healthcheck→result — *REQ-TUI-1*. **Done**: FAIL.
- [ ] T-068 — **(GREEN)** `internal/tui/model.go` root `Update` transitions all states correctly — *REQ-TUI-1*. **Done**: T-067 passes.
