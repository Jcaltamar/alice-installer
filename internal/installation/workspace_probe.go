...
func (p WorkspaceProbe) Probe(context.Context) ProbeResult {
	...
	for _, item := range infos {
		if item.err != nil && !os.IsNotExist(item.err) {
			return workspaceResult(PresenceUncertain, EvidenceWorkspaceUnreadable, "workspace artifacts cannot be validated", item.path)
		}
	}
	for _, item := range infos {
		if os.IsNotExist(item.err) {
			return workspaceResult(PresenceUncertain, EvidenceWorkspacePartial, "workspace artifacts cannot be validated", item.path)
		}
