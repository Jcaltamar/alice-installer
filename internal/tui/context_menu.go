...
type BackupPreflightCompletedMsg struct {
	Plan migration.BackupPlan
	Err  error
}
...
// LegacyBackupAction is the only migration capability the TUI may invoke.
type LegacyBackupAction interface {
	Preflight(context.Context, migration.BackupRequest) (migration.BackupPlan, error)
...
func isNormalAbsenceEvidence(kind installation.EvidenceKind) bool {
	...
	case installation.EvidenceWorkspaceAbsent,
		...
		installation.EvidenceLegacyDirectoryAbsent,
		installation.EvidenceLegacyDirectoryUnsupported:
		return true
	default:
		return false
	}
...
func safeEvidenceLabel(e installation.Evidence) string {
	labels := map[installation.EvidenceKind]string{
		...
		installation.EvidenceLegacyDirectory:            "Known legacy Alice directory found",
		installation.EvidenceLegacyDirectoryAbsent:      "Known legacy Alice directory not found",
		installation.EvidenceLegacyDirectoryInvalid:     "Known legacy Alice path is not a directory",
		installation.EvidenceLegacyDirectoryUnreadable:  "Known legacy Alice directory could not be inspected",
		installation.EvidenceLegacyDirectoryUnsupported: "Legacy directory probing is unsupported on this platform",
	}
